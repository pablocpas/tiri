...
pub struct Niri {
    ...
    pub config_file_output_config: tiri_config::Outputs,
    ...
    pub pick_color: Option<async_channel::Sender<Option<tiri_ipc::PickedColor>>>,
...
impl CastTarget {
    ...
    pub fn matches(&self, ipc: &tiri_ipc::CastTarget) -> bool {
        use CastTarget::*;
        match (self, ipc) {
            (Nothing, tiri_ipc::CastTarget::Nothing {}) => true,
            (Output { name, .. }, tiri_ipc::CastTarget::Output { name: ipc_name }) => {
                name == ipc_name
            }
            (Window { id }, tiri_ipc::CastTarget::Window { id: ipc_id }) => id == ipc_id,
            _ => false,
        }
    }

    pub fn make_ipc(&self) -> tiri_ipc::CastTarget {
        use CastTarget::*;
        match self {
            Nothing => tiri_ipc::CastTarget::Nothing {},
            Output { name, .. } => tiri_ipc::CastTarget::Output { name: name.clone() },
            Window { id } => tiri_ipc::CastTarget::Window { id: *id },
        }
    }
}
...
impl State {
        ...
        *CHILD_ENV.write().unwrap() = mem::take(&mut config.environment);
        ...
        if new_mod_key != self.backend.mod_key(&old_config) || binds_changed || modes_changed {
            ...
            self.niri.mods_with_tablet_stylus_binds =
                mods_with_tablet_stylus_binds(new_mod_key, active_binds);
    ...
    where
        F: FnOnce(&mut tiri_config::Output),
    {
        ...
        let config = if let Some(config) = config.outputs.find_mut(match_name) {
            ...
            config.outputs.0.push(tiri_config::Output {
    ...
    pub fn apply_transient_output_config(&mut self, name: &str, action: tiri_ipc::OutputAction) {
        self.modify_output_config(name, move |config| match action {
            tiri_ipc::OutputAction::Off => config.off = true,
            tiri_ipc::OutputAction::On => config.off = false,
            tiri_ipc::OutputAction::Mode { mode } => {
                config.mode = match mode {
                    tiri_ipc::ModeToSet::Automatic => None,
                    tiri_ipc::ModeToSet::Specific(mode) => Some(tiri_config::output::Mode {
                        custom: false,
                        mode,
                    }),
                };
            ...
            tiri_ipc::OutputAction::CustomMode { mode } => {
                config.mode = Some(tiri_config::output::Mode { custom: true, mode });
                config.modeline = None;
            }
            tiri_ipc::OutputAction::Modeline {
                clock,
                hdisplay,
                hsync_start,
                hsync_end,
                ...
                config.modeline = Some(tiri_config::output::Modeline {
                    clock,
                    hdisplay,
                    hsync_start,
                    hsync_end,
            ...
            tiri_ipc::OutputAction::Scale { scale } => {
                config.scale = match scale {
                    tiri_ipc::ScaleToSet::Automatic => None,
                    tiri_ipc::ScaleToSet::Specific(scale) => Some(FloatOrInt(scale)),
                }
            }
            tiri_ipc::OutputAction::Transform { transform } => config.transform = transform,
            tiri_ipc::OutputAction::Position { position } => {
                config.position = match position {
                    tiri_ipc::PositionToSet::Automatic => None,
                    tiri_ipc::PositionToSet::Specific(position) => Some(tiri_config::Position {
                        x: position.x,
                        y: position.y,
                    }),
                }
            }
            tiri_ipc::OutputAction::Vrr { vrr } => {
                config.variable_refresh_rate = if vrr.vrr {
                    Some(tiri_config::Vrr {
            ...
            tiri_ipc::OutputAction::MaxBpc { max_bpc } => {
                config.max_bpc = Some(tiri_config::output::MaxBpc(max_bpc));
            }
        });
    ...
    pub fn handle_pick_color(&mut self, tx: async_channel::Sender<Option<tiri_ipc::PickedColor>>) {
        let pointer = self.niri.seat.get_pointer().unwrap();
    ...
    pub fn on_introspect_msg(
        ...
        windows.insert(
            ...
            gnome_shell_introspect::WindowProperties {
                title: String::from("tiri Dynamic Cast Target"),
                app_id: String::from("rs.bxt.tiri.desktop"),
            },
        );
...
impl Niri {
    ...
    pub fn new(
        ...
        let mods_with_tablet_stylus_binds = mods_with_tablet_stylus_binds(mod_key, active_binds);
    ...
    pub fn inhibit_power_key(&mut self) -> anyhow::Result<()> {
        ...
        let message = conn.call_method(
            ...
            "Inhibit",
            &("handle-power-key", "tiri", "Power key handling", "block"),
        )?;
    ...
    pub fn reposition_outputs(&mut self, new_output: Option<&Output>) {
        ...
        struct Data {
            ...
            config: Option<tiri_config::Position>,
        }
    ...
    pub fn set_binding_mode(&mut self, mode: String, mod_key: ModKey) {
        ...
        self.mods_with_tablet_stylus_binds = mods_with_tablet_stylus_binds(mod_key, active_binds);
