...
use tiri_config::{FloatOrInt, OutputName, Vrr};
use tiri_ipc::Transform;
...
pub struct OutputManagementManagerState {
    ...
    current_state: HashMap<OutputId, tiri_ipc::Output>,
    current_config: tiri_config::Outputs,
}
...
pub trait OutputManagementHandler {
    ...
    fn apply_output_config(&mut self, config: tiri_config::Outputs);
}
...
enum OutputConfigurationState {
    Ongoing(HashMap<OutputId, tiri_config::Output>),
    Finished,
}
...
impl OutputManagementManagerState {
    ...
    pub fn on_config_changed(&mut self, new_config: tiri_config::Outputs) {
        self.current_config = new_config;
    }

    pub fn notify_changes(&mut self, new_state: HashMap<OutputId, tiri_ipc::Output>) {
                ...
                match new_config.entry(*output) {
                    ...
                    Entry::Vacant(entry) => {
                        ...
                        let mut config = g_state
                            ...
                            .find(&name)
                            .cloned()
                            .unwrap_or_else(|| tiri_config::Output {
    ...
    fn request(
        ...
        match request {
            zwlr_output_configuration_head_v1::Request::SetMode { mode } => {
                ...
                new_config.mode = Some(tiri_config::output::Mode {
                    custom: false,
                    mode: tiri_ipc::ConfiguredMode {
                        width: mode.width,
                        height: mode.height,
            ...
            zwlr_output_configuration_head_v1::Request::SetPosition { x, y } => {
                new_config.position = Some(tiri_config::Position { x, y });
            }
