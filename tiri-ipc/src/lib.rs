//! Types for communicating with tiri via IPC.
//!
//! After connecting to the tiri socket, you can send [`Request`]s. Tiri will process them one by
//! one, in order, and to each request it will respond with a single [`Reply`], which is a `Result`
//! wrapping a [`Response`].
//!
//! If you send a [`Request::EventStream`], tiri will *stop* reading subsequent [`Request`]s, and
...
//!
//! 1. Read the socket filesystem path from [`socket::SOCKET_PATH_ENV`] (`$TIRI_SOCKET`).
...
//! 3. Tiri will respond with a single line JSON-formatted [`Reply`].
//! 4. You can keep writing [`Request`]s, each on a single line, and read [`Reply`]s, also each on a
//!    separate line.
//! 5. After you request an event stream, tiri will keep responding with JSON-formatted [`Event`]s,
//!    on a single line each.
//!
//! ## Backwards compatibility
//!
//! This crate follows the tiri version. It is **not** API-stable in terms of the Rust semver. In
...
//!   the types.
//! - `clap`: derives the clap CLI parsing traits for some types. Used internally by tiri itself.
#![warn(missing_docs)]
...
/// Request from client to tiri.
#[derive(Debug, Serialize, Deserialize, Clone)]
#[cfg_attr(feature = "json-schema", derive(schemars::JsonSchema))]
pub enum Request {
    /// Request the version string for the running tiri instance.
    Version,
...
/// Successful response from tiri to client.
#[derive(Debug, Serialize, Deserialize, Clone)]
#[cfg_attr(feature = "json-schema", derive(schemars::JsonSchema))]
pub enum Response {
    /// A request that does not need a response was handled successfully.
    Handled,
    /// The version string for the running tiri instance.
    Version(String),
...
/// Actions that tiri can perform.
// Variants in this enum should match the spelling of the ones in tiri-config. Most, but not all,
// variants from tiri-config should be present here.
#[derive(Serialize, Deserialize, Debug, Clone)]
...
/// Output actions that tiri can perform.
// Variants in this enum should match the spelling of the ones in tiri-config. Most thigs from
// tiri-config should be present here.
#[derive(Serialize, Deserialize, Debug, Clone)]
#[cfg_attr(feature = "clap", derive(clap::Parser))]
#[cfg_attr(feature = "clap", command(subcommand_value_name = "ACTION"))]
#[cfg_attr(feature = "clap", command(subcommand_help_heading = "Actions"))]
...
pub enum OutputAction {
    ...
    Mode {
        ...
        ///
        /// Run `tiri msg outputs` to see the available modes.
        #[cfg_attr(feature = "clap", arg())]
        mode: ModeToSet,
    },
...
#[cfg_attr(feature = "json-schema", derive(schemars::JsonSchema))]
pub enum ModeToSet {
    /// Tiri will pick the mode automatically.
    Automatic,
    /// Specific mode.
...
#[cfg_attr(feature = "json-schema", derive(schemars::JsonSchema))]
pub enum ScaleToSet {
    /// Tiri will pick the scale automatically.
    Automatic,
    /// Specific scale.
    Specific(f64),
}
...
/// some optional properties are present or absent for certain window types may change across tiri
/// releases.
///
...
pub struct WindowLayout {
    ...
    ///
    /// Does not include tiri decorations like borders.
    ///
    ...
    ///
    /// This is the same "workspace view" as in gradients' `relative-to` in the tiri config.
