...
There's [a bug](https://github.com/wezterm/wezterm/issues/4708) in WezTerm that it waits for a zero-sized Wayland configure event, so its window never shows up in tiri. To work around it, put this window rule in the tiri config (included in the default config):
...
So if you hit this problem, comment out `prefer-no-csd` in the tiri config and restart WezTerm.
...
For some reason, DMABUF screencasts are disabled in the Zen Browser, so screencasting doesn't work out of the box on tiri.
...
Note that the tiri environment config does not propagate to apps and shells started by systemd, for example to DankMaterialShell and its application launcher.
You can set the variable in your login shell config (i.e. `~/.bash_profile`) instead, though keep in mind that then it will be set for all compositors, not just tiri.
...
You can move them to a more convenient location by adding a window rule in your tiri config:
...
This leads to tiri filling the transparent pixels inside the corners with black.
